...
shared_ptr<rtc::PeerConnection> _createPeerConnection(const rtc::Configuration &config,
                                                     weak_ptr<rtc::WebSocket> wws, std::string id);

std::string _randomId(size_t length);
...
int main(int argc, char **argv) try {
    ...
    _localId = _randomId(4);
    std::cout << "The local ID is " << _localId << std::endl;
    ...
    ws->onMessage([&config, wws = make_weak_ptr(ws)](auto data) {
        ...
        if (auto jt = peerConnectionMap.find(id); jt != peerConnectionMap.end()) {
            ...
            pc = _createPeerConnection(config, wws, id);
        } else {
            return;
        }
    ...
    const std::string url = wsPrefix + params.webSocketServer() + ":" +
                            std::to_string(params.webSocketPort()) + "/" + _localId;
    ...
    while (true) {
        ...
        auto pc = _createPeerConnection(config, ws, id);
